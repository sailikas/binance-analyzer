...
✅ **定时任务**: 支持后台定时运行，可自定义间隔时间（分钟+秒）  
✅ **多通知方式**: 支持Android原生通知、plyer、Server酱（微信推送）三种方式同时发送  
✅ **智能通知**: 发现新币种或币种消失时自动推送通知，显示前3名币种详情  
✅ **历史记录**: SQLite数据库存储所有分析结果，支持按时间筛选（2天/7天/30天/全部）  
✅ **参数配置**: 可自定义最小涨幅、流动性阈值、最大分析数量等参数  
✅ **Server酱集成**: 支持微信推送，可自定义通知标题和正文模板  
✅ **Bilibili风格UI**: 采用B站+Element UI配色方案，底部导航栏设计，操作简洁流畅  
